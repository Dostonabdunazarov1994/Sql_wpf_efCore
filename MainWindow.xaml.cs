    ...
    public partial class MainWindow : Window
    {
        ...
        private void textboxSearch_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                using (MyDbContext context = new MyDbContext())
                {
                    datagrid1.ItemsSource = context.Movies.Where(b => b.Movie_Name.Contains(textboxSearch.Text)).ToList();
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
        ...
        private void textboxsearch2_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                using (MyDbContext context = new MyDbContext())
                {
                    datagrid2.ItemsSource = context.Halls.Where(b => b.Hall_Name.Contains(textboxsearch2.Text)).ToList();
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
